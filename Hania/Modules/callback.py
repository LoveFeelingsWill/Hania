...
from Hania import YouTube, app
from Hania.core.call import hania
from Hania.misc import SUDOERS, db
from Hania.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from Hania.utils.decorators.language import languageCB
from Hania.utils.formatters import seconds_to_min
from Hania.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from Hania.utils.stream.autoclear import auto_clean
from Hania.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await hania.pause_stream(chat_id)
        ...
        await hania.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await hania.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await hania.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await hania.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await hania.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await hania.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
